...
public final class RegistrazioneController {
    ...
    public Boolean scegliRegistrazione(final @RequestBody
                                              RequestRegister request) {

        //verifica che l'email non è registrato
        if(request.getRole().equalsIgnoreCase("biblioteca")){
            if(registrazioneService.getBibliotecaByEmail(request.email) !=null)
                return false;
            else {

                registrazioneService.registraBiblioteca(new Biblioteca(request.getEmail(),request.getPassword(),"","","","",""));
                return true;
            }
        }

        if(request.getRole().equalsIgnoreCase("esperto")){
            //replica per esperto e controlla che non ci siano altre mail
        }

        if(request.getRole().equalsIgnoreCase("lettore")){
